...
if __name__ == "__main__":
    ...
    # Load airport list ...
    with open(f"{fmc.__path__[0]}/db.json", mode = "rt", encoding = "utf-8") as fObj:
        airports = json.load(fObj)

    # Create list of Estonian, Finnish, Latvian, Lithuanian and Swedish "large"
    # airfields within the field-of-view (where "large" means "has an IATA code") ...
    airfields = []
    for airport in airports:
        ...
        if skip:
            continue
        if airport["ISO 3166-1 Alpha-3"] in [
            "EST",
            "FIN",
            "LVA",
            "LTU",
            ...
            if fov.contains(pnt):
                airfields.append(
                    (
                        airport["name"].removesuffix(" Airfield").removesuffix(" Airport").removesuffix(" Air Base"),
                        airport["IATA"],
                        pnt,
                    )
                )
            else:
                print(f'INFO: \"{airport["name"]} ({airport["IATA"]})\", in \"{airport["ISO 3166-1 Alpha-3"]}\", is not within the field-of-view.')
    ...
    for dist in range(distMin, distMax + 1, 1):
        ...
        ax.scatter(
            [pnt.x for _, _, pnt in airfields],
            [pnt.y for _, _, pnt in airfields],
                color = "cyan",
               marker = "x",
        ...
        # Loop over airfields ...
        for name, iata, pnt in airfields:
            # Annotate airfield (shifting labels up/down/left/right if point is
            # up/down/left/right of centre) ...
            pyguymer3.geo.add_annotation(
                ...
                arrowprops = {
                    "arrowstyle" : "-",
                     ...
                     debug = args.debug,
                    txtLon = pnt.x - 1.0 if pnt.x < midLon else pnt.x + 1.0,
                    txtLat = pnt.y - 0.5 if pnt.y < midLat else pnt.y + 0.5,
                    zorder = 5.0,       # This is the same as the airfield markers.
            )
            pyguymer3.geo.add_annotation(
                ax,
                pnt.x - 1.0 if pnt.x < midLon else pnt.x + 1.0,
                pnt.y - 0.5 if pnt.y < midLat else pnt.y + 0.5,
                f"{name}\n({iata})",
                    bbox = {
                    "edgecolor" : "black",
                    "facecolor" : "white",
                   ...
                   debug = args.debug,
                fontsize = 4,
                  zorder = 6.0,         # This is above the airfield markers and airfield arrows.
            )
    ...
    for maxSize in maxSizes:
        ...
        shutil.move(vname, f"{outDir}/lon={lon:+011.6f}_lat={lat:+010.6f}_Baltic{maxSize:04d}px.mp4")
